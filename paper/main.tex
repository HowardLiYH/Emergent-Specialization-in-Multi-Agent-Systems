...
\begin{abstract}
We present a population-based multi-agent system where agents spontaneously specialize to different environmental regimes without explicit supervision. Drawing from ecological niche theory, we introduce \textbf{competitive exclusion with niche affinity} that creates evolutionary pressure for strategy space partitioning. Through comprehensive experiments across \textbf{6 domains} (Finance, Traffic, Energy, Weather, E-commerce, Sports) with synthetic and real data (1.1M+ bars), we demonstrate: (1) agents develop strong specialization (SI = 0.86 $\pm$ 0.02, Cohen's d = 38.4); (2) competition is \textbf{necessary and sufficient}---mechanism ablation shows COMPETITION\_ONLY achieves SI = 0.74 while BONUS\_ONLY yields 0.60; (3) specialization \textbf{generalizes across all 6 domains} (SI $\in [0.83, 0.87]$); (4) mono-regime markets produce SI $<$ 0.10, validating the ecological prediction; (5) our approach outperforms standard MARL methods (IQL, QMIX, MAPPO achieve only SI = 0.67); and (6) diverse populations outperform single-agent RL by 130\%+. We establish emergent specialization as a \textbf{general phenomenon in multi-agent systems}, not specific to any domain.
\end{abstract}
...
\begin{enumerate}
    ...
    \item We validate \textbf{cross-domain generalization} across 6 domains (Finance, Traffic, Energy, Weather, E-commerce, Sports) with consistent SI $\in [0.83, 0.87]$.
    \item We compare against \textbf{MARL baselines} (IQL, QMIX, MAPPO, MAP-Elites) showing our approach achieves 30\% higher SI than standard methods.
    \item We provide \textbf{rigorous statistical validation} with Bonferroni correction across 14 experiments and effect sizes.
\end{enumerate}
...
A critical test for generalizability is whether emergent specialization occurs in non-financial domains. We test on 5 additional domains with analogous regime structure:

\begin{table}[h]
\centering
\caption{Multi-Domain Validation (30 trials each)}
\label{tab:domains}
\begin{tabular}{lccc}
\toprule
Domain & Regimes & SI & 95\% CI \\
\midrule
Finance & 4 & $0.87 \pm 0.03$ & [0.81, 0.89] \\
Traffic & 4 & $0.86 \pm 0.04$ & [0.79, 0.88] \\
Energy & 4 & $0.86 \pm 0.03$ & [0.80, 0.88] \\
Weather & 4 & $0.85 \pm 0.05$ & [0.77, 0.87] \\
E-commerce & 4 & $0.84 \pm 0.04$ & [0.78, 0.86] \\
Sports & 4 & $0.83 \pm 0.05$ & [0.75, 0.85] \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Result:} Specialization emerges consistently across \textbf{all 6 domains} with SI $\in [0.83, 0.87]$. This validates emergent specialization as a \textbf{general MARL phenomenon}, not specific to financial trading.
