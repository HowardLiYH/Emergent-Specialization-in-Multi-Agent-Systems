...
\textbf{Key findings:} (1) Specialization emerges strongly on real data (SI = 0.879); (2) However, the homogeneous baseline outperforms diverse population on this dataset. This reveals an important insight: \textbf{specialization emergence does not guarantee superior performance on all market structures}. Real markets may favor simpler strategies, or HMM-detected regimes may not align with optimal strategy boundaries.
...
\begin{enumerate}
    \item \textbf{Emergence is robust:} Agents spontaneously partition the regime space (SI = 0.86), achieving near-perfect specialist win rates (80\%), even without explicit incentives ($\lambda = 0$ yields SI = 0.59).
    \item \textbf{Diversity has value:} Diverse populations outperform homogeneous baselines by 7.4\% on synthetic data, validating emergent diversity.
    \item \textbf{Superior to single-agent RL:} Multi-agent specialization outperforms DQN by 132\% and PPO by 2063\%, demonstrating clear benefits of population-based learning.
    \item \textbf{Real market challenges:} While specialization emerges on real data (SI = 0.879), performance benefits require careful regime detection and strategy alignment.
    \item \textbf{Robust to hyperparameters:} Specialization is stable across exploration rates (0.05--0.30) and inventory sizes (4--12).
\end{enumerate}

Future work will develop adaptive regime detection that learns strategy-aligned boundaries, explore transfer to other domains, and investigate market-making applications where specialization could provide competitive advantages.
