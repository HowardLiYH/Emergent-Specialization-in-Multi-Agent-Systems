...
Multi-agent coordination remains one of the fundamental challenges in artificial intelligence. When multiple agents operate in shared environments, they face a critical dilemma: how to divide labor and specialize without explicit communication. This question connects to a deeper debate about the nature of intelligence itself: recent exchanges between AI pioneers have reignited questions about whether intelligence is inherently ``general'' or fundamentally specialized \cite{lecun2025twitter, hassabis2025twitter}. LeCun argues that human intelligence is ``ridiculously specialized''---we can only compute an infinitesimal fraction of possible functions, and our sense of generality is an illusion. Hassabis counters that brains are general in the Turing-complete sense, though he acknowledges that ``some degree of specialisation around the target distribution'' is necessary. Our work contributes empirical evidence to this debate: we show that \textbf{specialization emerges inevitably from competition}, suggesting that the tendency to specialize is itself a universal property of bounded intelligent systems.
...
The question of whether intelligence is inherently general or specialized has resurfaced in recent debates among AI leaders. LeCun argues that human intelligence is ``super-specialized for the physical world'' and that our perception of generality is illusory---we cannot imagine the problems we are blind to \cite{lecun2025twitter}. He provides a compelling mathematical argument: the space of possible boolean functions on sensory inputs ($2^{2^{10^6}}$) vastly exceeds what any brain can represent ($2^{3.2 \times 10^{15}}$), making biological intelligence ``ridiculously specialized'' by necessity. Hassabis counters that this conflates generality with universality: in the Turing Machine sense, brains are capable of learning anything computable given sufficient resources \cite{hassabis2025twitter}. However, he acknowledges that practical systems require ``some degree of specialisation around the target distribution.''
