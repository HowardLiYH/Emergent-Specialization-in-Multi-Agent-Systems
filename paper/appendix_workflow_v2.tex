...
\begin{figure}[h!]
    ...
    \node (alpha) [init, right=2cm of beta] {Init $\alpha_{i,r} = 1/R$\\$[0.25, 0.25, 0.25, 0.25]$};

    % === PHASE 1: CONTEXT (centered below the two init nodes) ===
    \node (regime) [process, below=1.8cm of beta, xshift=2.5cm] {Sample regime $r_t \sim \pi(r)$\\Example: $r_t = \text{Bull}$};
    ...
    \node (skillup) [winner, below=1.5cm of compare, xshift=-2cm] {Skill update:\\$\beta^+_{r_t,m_i} \mathrel{+}= 1$};
    \node (affup) [winner, below=1.5cm of compare, xshift=2cm] {Affinity update:\\$\alpha_{r_t} \mathrel{+}= \eta(1-\alpha_{r_t})$};
    \node (normalize) [winner, below=0.8cm of compare, yshift=-2.5cm] {Normalize: $\alpha \leftarrow \alpha / \|\alpha\|_1$};

    % === LOOP BACK ===
    \node (nextiter) [process, below=0.8cm of normalize] {Next iteration $t \leftarrow t+1$};

    % === ARROWS ===
    \draw [arrow] (beta.south) -- ++(0,-0.5) -| (regime.north);
    \draw [arrow] (alpha.south) -- ++(0,-0.5) -| (regime.north);
    ...
    \draw [arrow] (compare.south) -- ++(0,-0.3) -| (skillup.north);
    \draw [arrow] (compare.south) -- ++(0,-0.3) -| (affup.north);
    \node [left=0.1cm of compare, yshift=-0.5cm, font=\footnotesize] {Yes};
    \draw [arrow] (skillup.south) |- (normalize.west);
    \draw [arrow] (affup.south) |- (normalize.east);
    \draw [arrow] (normalize) -- (nextiter);
    \draw [arrow, rounded corners] (nextiter.west) -- ++(-4,0) |- (regime.west);

    % Phase labels
    \node [above=0.3cm of beta, font=\footnotesize\bfseries, blue!70] {PHASE 0: INIT};
    \node [left=0.3cm of regime, font=\footnotesize\bfseries, rotate=90, anchor=south] {PHASE 1};
    \node [left=0.3cm of thompson, font=\footnotesize\bfseries, rotate=90, anchor=south] {PHASE 2};
    \node [left=0.3cm of execute, font=\footnotesize\bfseries, rotate=90, anchor=south] {PHASE 3};
    \node [left=0.3cm of bonus, font=\footnotesize\bfseries, rotate=90, anchor=south] {PHASE 4};
    \node [left=0.3cm of skillup, font=\footnotesize\bfseries, rotate=90, anchor=south, green!50!black] {PHASE 5};
