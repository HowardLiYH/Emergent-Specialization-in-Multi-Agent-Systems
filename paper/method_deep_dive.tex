...
\begin{itemize}
    \item \textbf{Metric}: Maximum agent reward during forced rare-regime evaluation (normalized 0--1 scale)
    \item \textbf{Protocol}: After training each population (500 iterations), we force the environment into rare regimes for 50 evaluation episodes
    \item \textbf{Reward}: Computed from the regime-method affinity matrix (Table 8--13) plus Gaussian noise ($\sigma=0.1$)
    \item \textbf{Comparison}: Best-performing agent's reward in each population type
\end{itemize}
...
\begin{itemize}
    \item \textbf{Weather/Extreme (+81.5\% vs Homo)}: During extreme weather (10\% of time), homogeneous populations using ``naive'' persistence achieve only 0.541 reward. NichePopulation develops an extreme-weather specialist using ``trend'' method, achieving 0.982 reward.
    \item \textbf{Why MARL underperforms}: MARL methods (IQL/VDN/QMIX/MAPPO) optimize joint reward, converging to strategies optimal for \textit{common} regimes. They lack specialists for rare regimes.
    \item \textbf{Percentage calculation}: $\frac{\text{Niche} - \text{Baseline}}{\text{Baseline}} \times 100\%$. E.g., Weather: $\frac{0.982 - 0.541}{0.541} = +81.5\%$
\end{itemize}

\textbf{Key findings}:
\begin{itemize}
    \item \textbf{NichePopulation outperforms ALL baselines} in rare regimes (4/6 domains significant, $p < 0.05$)
    \item vs Homogeneous: \textbf{+21.5\%} average improvement in rare-regime reward
    \item vs MARL methods: +15.6\% to +16.1\% average improvement
    \item \textbf{Root cause}: MARL produces near-homogeneous populations (SI $\approx$ 0.12--0.20), lacking rare-regime specialists
